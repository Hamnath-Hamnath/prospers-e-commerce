...
const ProductScreen = (props: Props) => {
  ...
  const productDetails: any = useSelector((state:any) => state.productDetails);
  const { loading, error, product }: any = productDetails;
  ...
  // Review
  const [rating, setRating] = useState<number | any>(0);
  const [comment, setComment] = useState<string | any>('');

  const productReviewCreate: any = useSelector(
    (state: any) => state.productReviewCreate
  );
  const {
    error: errorReview,
    success: successReview,
  }: any = productReviewCreate;
  ...
  useEffect(() => {
    ...
    //   const x:[]|any = await getProduct(id);
    //   setProduct(x);
    // };
    // response();
